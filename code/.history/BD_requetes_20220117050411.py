...
def addSwitch():
    #Initialisation d'utilisateur
    db = OpenDatabase()
    req = 'INSERT INTO switches (switch_name, switch_state, lastname, firstname, mail, pass) values (?, ?, ?, ?, ?, ?)'
    ExecuteDatabase(db, req,[newUser['role'],newUser['matricule'],newUser['lastname'],newUser['firstname'], newUser['mail'], newUser['password']])
    CloseDatabase(db)
    return 'ok'
...
def updateSwitchState(switchesState):
    ...
    for switch in switchesState:
        ExecuteDatabase(db, 'UPDATE switch_state FROM switches WHERE switch_name = switch')
    CloseDatabase(db)
    return "ok"
