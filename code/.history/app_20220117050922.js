...
$('#btn-vacuum-pump').on('change', function(){RefreshSwitchState();});
$('#btn-cooling-pump').on('change', function(){RefreshSwitchState();});
$('#btn-heater').on('change', function(){RefreshSwitchState();});
$('#btn-solenoid-valve').on('change', function(){RefreshSwitchState();});

function RefreshSwitchState(){
  var vacuum_pump_state = "OFF";
  var cooling_pump_state = "OFF";
  var heater_state = "OFF";
  var solenoid_valve_state = "OFF";
  if($('#btn-vacuum-pump').is(":checked")){vacuum_pump_state="ON";}
  else{vacuum_pump_state="OFF";}
  if($('#btn-cooling-pump').is(":checked")){cooling_pump_state="ON";}
  else{cooling_pump_state="OFF";}
  if($('#btn-heater').is(":checked")){heater_state="ON";}
  else{heater_state="OFF";}
  if($('#btn-solenoid-valve').is(":checked")){solenoid_valve_state="ON";}
  else{solenoid_valve_state="OFF";}
  
  console.log(vacuum_pump_state);
  $.ajax({
    type: "POST",
    url: "/switch-state",
    data: JSON.stringify({
      vacuum_pump: vacuum_pump_state,
      cooling_pump: cooling_pump_state,
      heater: heater_state,
      solenoid_valve: solenoid_valve_state,
    }),
    success: function (s) {
      console.log("ok");
    },
