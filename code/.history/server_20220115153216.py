...
def login():
    if request.method == 'GET':
        ...
        print(bcrypt.check_password_hash(bdinfos[0][6], query['password']))
        # Vérification du mot de passe
        if ( state == True and bcrypt.check_password_hash(bdinfos[0][6], query['password'])):
            session['username'] = bdinfos[0][5]
            session['user_id'] = bdinfos[0][0]
            session['role'] = bdinfos[0][1]
            session['matricule'] = bdinfos[0][2]
            return return redirect('home', code = 302)
        else:
            abort(400)  
