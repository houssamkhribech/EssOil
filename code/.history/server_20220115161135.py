...
def login():
    if request.method == 'GET':
        ...
        if ( state == True and bcrypt.check_password_hash(DB_[0][6], query['password'])):
            session['username'] = DB_[0][5]
            """session['user_id'] = DB_[0][0]
            session['role'] = DB_[0][1]
            session['matricule'] = DB_[0][2]"""
            return redirect('home', code = 302)
        else:
            abort(400)  
    else: 
