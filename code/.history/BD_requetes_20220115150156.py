...
def Inituser(pass1, pass2):
    ...
    db = OpenDatabase()
    pw_hash = bcrypt.generate_password_hash('Houssam').decode('utf-8')    # hashage du mot de passe
    req = 'INSERT INTO users (role, matricule, lastname, firstname, mail, pass) values (0, "B2200145", "Khribech", "Houssam", "H.Khribech@HE.com", ?)'
    ExecuteDatabase(db, req, [pass])
    pw_hash = bcrypt.generate_password_hash('Youssef').decode('utf-8')    # hashage du mot de passe
    ...
    ExecuteDatabase(db, req, [pw_hash])
    CloseDatabase(db)
################################
